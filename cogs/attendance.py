...
class SundayChoiceButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Лог
        log_embed = discord.Embed(
            title="📝 Выбор воскресного КВ",
            description=f"Событие: **{self.event_name}**",
            color=discord.Color.blue()
        )
        log_embed.add_field(name="Выбрал", value=interaction.user.mention)
        log_embed.add_field(name="Дата", value=format_date(date_str=self.date))
        await self.bot.send_log(interaction.guild, log_embed)
